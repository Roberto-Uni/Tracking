    ...
    def update(self, objects_rect):
        ...
        if not self.first:
            for id, (x, y, w, h) in enumerate(objects_rect):
              cx = x
              cy = y
              self.center_points[id] = [cx, cy, 0.875>y]
              self.memory[id] = 0
        ...
        for rect in objects_rect:
            ...
            for id, pt in self.center_points.items():
                dist.append((math.hypot(cx - pt[0], cy - pt[1]), id))

            d, id = min(dist)
            if d < 0.03:
                self.center_points[id] = [cx, cy, self.center_points[id][2]]
                self.memory[id] = 0
                objects_bbs_ids.append([x, y, w, h, id])
            ...
            if same_object_detected is False:
                self.center_points[self.id_count] = [cx, cy, 0.875>cy]
        ...
        for id, count in self.memory.items():
          if count < 10 and id not in new_center_points:
